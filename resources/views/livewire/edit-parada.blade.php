<div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-lg">
    <h2 class="text-2xl font-semibold text-center mb-6">Edit Parada</h2>
    ...
    <form wire:submit.prevent="save">
       ...
       <div class="mb-4">
        ...
        <div class="mb-4">
            <label for="ubicacio" class="block text-sm font-medium text-gray-700">Ubicacio:<span class="text-red-500">*</span></label>
            <input type="text" id="ubicacio" wire:model="ubicacio" maxlength="150" required 
                class="mt-1 p-2 w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            @error('ubicacio') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <!-- Mostrar imágenes existentes -->
                ...
                <div class="mb-4">
                    <h4 class="text-sm font-medium text-gray-600 mb-2">Imatges existents:</h4>
                    <div class="grid grid-cols-3 gap-2">
                        @foreach($existing_images as $key => $image)
                            <div class="relative p-2 bg-gray-50 border border-gray-200 rounded-xl">
                                <img src="{{ $image['path'] }}" class="mb-2 w-full h-32 object-cover rounded-lg">
                                <p class="text-xs text-gray-500 text-center">{{ $image['name'] }}</p>
                                <button type="button" wire:click="removeExistingImage({{ $key }})" 
            ...
            @if (!empty($imatges) && is_array($imatges))
                <div class="mt-2">
                    <h4 class="text-sm font-medium text-gray-600 mb-2">Noves imatges:</h4>
                    <div class="grid grid-cols-3 gap-2">
        ...
        <div class="mb-4">
            ...
            @if (!empty($existing_files) && is_array($existing_files))
                <div class="mb-4">
                    <h4 class="text-sm font-medium text-gray-600 mb-2">Fitxers existents:</h4>
                    <div class="space-y-2">
                        @foreach($existing_files as $file)
                            <div class="relative p-2 bg-gray-50 border border-gray-200 rounded-xl">
                                <div class="flex items-center gap-x-3">
                                    ...
                                    <span class="text-sm text-gray-800">{{ $file['nom'] }}</span>
                                 
                                </div>
                                <button type="button" wire:click="removeExistingFile({{ $key }})" 
                                    class="absolute top-1 right-1 text-gray-500 hover:text-red-600 focus:outline-none">
                                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" 
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <circle cx="12" cy="12" r="10"></circle>
            ...
            <!-- Preview para nuevos archivos -->
            @if (!empty($fitxers_adjuntats) && is_array($fitxers_adjuntats))
                <div class="mt-2">
                    <h4 class="text-sm font-medium text-gray-600 mb-2">Nous fitxers:</h4>
                    <div class="space-y-2">
